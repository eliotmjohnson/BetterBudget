...
The application installs as **Better Budget** with white-background `any` and maskable icons, white theme/background metadata, safe-area support, and standalone display configuration. The installed iOS app runs edge to edge under the status bar (`statusBarStyle: 'black-translucent'` in `src/app/layout.tsx`), so every top-anchored surface pads itself by `env(safe-area-inset-top)`, and full-height surfaces, starting with `html` and `body`, use `--viewport-height` from `tokens.css` (`100lvh` in the installed app), because iOS otherwise sizes the installed app's viewport a status bar short and leaves a band at the bottom. iOS reads the status-bar style when the app is added to the home screen, so a change to it only appears after removing and re-adding the app. To verify installation behavior, use a production build or a browser environment that permits local PWA installation; installed icon changes can remain cached by iOS and may require removing and reinstalling the home-screen app.
