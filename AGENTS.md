...
- The mobile PWA intentionally disables page zoom, text selection, touch callouts, document-level pull-to-refresh, and cancelable Safari history gestures beginning within the leftmost 24 px on any route. The global touch guard must remain capture-phase and non-passive for touchstart and touchmove, prevent non-control edge touches immediately, preserve normal control taps, and prevent rightward control drags once their direction is clear, including diagonals with vertical movement up to four times their horizontal movement. Mount it once above all routes. `.app-content` is the vertical overscroll surface for every route, continues behind the translucent blurred bottom navigation, and must retain enough bottom padding/scroll padding to expose the final content above that navigation. The top header remains opaque white, and route content enters over 0.65 s with the established blur-to-sharp motion.
